...
**只做这一件事**:把 `sqlite-source/src/util.c` 中的 `实现 src/util/pattern.rs: sqlite3_strglob(const char *zGlob, const char *zStr) -> i32, sqlite3_strlike(const char *zPattern, const char *zStr, u32 esc) -> i32。支持通配符 * 和 ?,以及 strlike 的 ESCAPE 字符。行为与 C 版 byte-for-byte 一致(包括大小写敏感性默认,以及 ESCAPE 0 行为)。` 用 1:1 Rust 行为等价地实现。
...
- **ID**:`T-0003`
- **模块**:`L0/util`
- **范围**:`实现 src/util/pattern.rs: sqlite3_strglob(const char *zGlob, const char *zStr) -> i32, sqlite3_strlike(const char *zPattern, const char *zStr, u32 esc) -> i32。支持通配符 * 和 ?,以及 strlike 的 ESCAPE 字符。行为与 C 版 byte-for-byte 一致(包括大小写敏感性默认,以及 ESCAPE 0 行为)。`
...
5. **不要扩大范围**:本任务只做 实现 src/util/pattern.rs: sqlite3_strglob(const char *zGlob, const char *zStr) -> i32, sqlite3_strlike(const char *zPattern, const char *zStr, u32 esc) -> i32。支持通配符 * 和 ?,以及 strlike 的 ESCAPE 字符。行为与 C 版 byte-for-byte 一致(包括大小写敏感性默认,以及 ESCAPE 0 行为)。,不要顺手"优化"或"顺便"重构别的。
